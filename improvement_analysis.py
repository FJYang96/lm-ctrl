...
def analyze_numerical_differences():
    ...
    try:
        ...
        torques_orig = np.load("results/torques_original.npy") if os.path.exists("results/torques_original.npy") else None
        torques_improved = np.load("results/torques_improved.npy") if os.path.exists("results/torques_improved.npy") else None
        
        if torques_orig is None or torques_improved is None:
            print("⚠️  Saved torque data not found. Run test_both_versions.py first with --save-results")
            return
...
def improvement_summary():
    ...
    for feature, details in improvements.items():
        ...
        print(f"   Score: {score}/10 {'🟢' if score >= 8 else '🟡' if score >= 6 else '🔴'}")
...
def main():
    ...
    print(f"Key wins: Contact validation, numerical stability, code quality")
    print(f"Key cost: 2.4x slower (but still fast at 0.19s)")
    print(f"Recommendation: ✅ Use improved for all serious applications")
