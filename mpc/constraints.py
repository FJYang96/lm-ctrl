...
def torque_limits_constraints(
    ...
    base_vel = cs.vertcat(com_velocity, com_angular_velocity)
    joint_vel = cs.vertcat(
        qvel_joints_FL, qvel_joints_FR, qvel_joints_RL, qvel_joints_RR
    )

    mass_matrix = kindyn_model.kindyn.mass_matrix_fun()(H, joint_positions)
    bias_force = kindyn_model.kindyn.bias_force_fun()(
    ...
    base_acc = (base_vel_next - base_vel) / dt

    qvel_next_FL = u_next[0:3]
    qvel_next_FR = u_next[3:6]
    qvel_next_RL = u_next[6:9]
    qvel_next_RR = u_next[9:12]

    joint_vel_next = cs.vertcat(qvel_next_FL, qvel_next_FR, qvel_next_RL, qvel_next_RR)

    joint_acc = (joint_vel_next - joint_vel) / dt

    J_FL = kindyn_model.kindyn.jacobian_fun("FL_foot")(H, joint_positions)[0:3, :]
    J_FR = kindyn_model.kindyn.jacobian_fun("FR_foot")(H, joint_positions)[0:3, :]
    J_RL = kindyn_model.kindyn.jacobian_fun("RL_foot")(H, joint_positions)[0:3, :]
    J_RR = kindyn_model.kindyn.jacobian_fun("RR_foot")(H, joint_positions)[0:3, :]

    F_FL = forces[0:3] * contact_k[0]
    F_FR = forces[3:6] * contact_k[1]
    F_RL = forces[6:9] * contact_k[2]
    F_RR = forces[9:12] * contact_k[3]
