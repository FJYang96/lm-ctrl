...
def solve_trajectory_optimization(
    ...
    # Get constraint hardness report if available (from slack formulation)
    ...
    if self.current_task_mpc is not None:
        ...
        print(f"🔧 [SLACK DEBUG] Slack formulation enabled: {use_slack}")

        # Log number of LLM constraints
        num_constraints = len(getattr(self.current_task_mpc, "constraint_functions", []))
        logger.info(f"Number of LLM constraints: {num_constraints}")
        print(f"🔧 [SLACK DEBUG] Number of LLM constraints: {num_constraints}")

        # Check if MPC is using slack
        if self.current_task_mpc.mpc is not None:
            mpc_type = type(self.current_task_mpc.mpc).__name__
            print(f"🔧 [SLACK DEBUG] MPC type: {mpc_type}")

        # Print objective value (score) if available
        if self.current_task_mpc.mpc is not None:
            ...
            if hasattr(mpc, "_last_solution") and mpc._last_solution is not None:
                try:
                    ...
                    pass
            # Also print slack penalty if available
        ...
        if hardness_report:
            ...
            for name, metrics_data in hardness_report.items():
                ...
                if max_slack > 0.01 and active_timesteps:
                    ...
                    logger.info(f"      Violated at: t={start_t:.2f}-{end_t:.2f}s ({len(active_timesteps)} timesteps)")

                    # Show worst timesteps
                    if slack_by_timestep:
                        worst = sorted(slack_by_timestep.items(), key=lambda x: -x[1])[:3]
                        worst_str = ", ".join([f"k={k}(t={k*mpc_dt:.2f}s, slack={v:.3f})" for k, v in worst if v > 1e-6])
                        if worst_str:
