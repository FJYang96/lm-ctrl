...
from .format_hardness import format_hardness_report, format_hardness_suggestions
...
def generate_failure_feedback(
    ...
    hardness_report: dict[str, dict] | None = None,
    mpc_dt: float = 0.02,
) -> str:
    """
    ...
    if hardness_report:
        lines.append(format_hardness_report(hardness_report, dt=mpc_dt))

        # Add specific suggestions based on hardness
        suggestions = format_hardness_suggestions(hardness_report, dt=mpc_dt)
        if suggestions:
            lines.append("\n" + "-" * 60)
            lines.append("SPECIFIC FIXES BASED ON CONSTRAINT HARDNESS")
            lines.append("-" * 60)
    ...
    if hardness_report:
        lines.append("PAY ATTENTION TO THE CONSTRAINT HARDNESS ANALYSIS ABOVE.")
        lines.append("Constraints marked 🔴 CRITICAL must have their bounds widened!")
