...
def format_hardness_report(
    hardness_report: dict[str, dict] | None,
    dt: float = 0.02,
    ...
    if system_constraints:
        lines.append(">>> SYSTEM CONSTRAINTS (for context - adjust your motion/contact to avoid) <<<")
        lines.append("")
        for name, metrics in system_constraints.items():
            ...
            if max_slack > 0.01:
                ...
                severity = "✅" if max_slack < 0.001 else "🟢"
                display_name = name.replace("_constraints", "")
                lines.append(f"  {severity} {display_name}: OK (max slack = {max_slack:.4f})")
        lines.append("")
...
def _format_constraint_detail(
    name: str, metrics: dict, dt: float, is_llm: bool
) -> list[str]:
    ...
    if active_timesteps:
        ...
        lines.append(f"      Violated at: {time_str} ({len(active_timesteps)} timesteps)")
...
def get_hardness_summary(hardness_report: dict[str, dict] | None) -> dict[str, Any]:
    """
    Get a summary of constraint hardness for metrics tracking.
    ...
    for name, metrics in hardness_report.items():
        slack = metrics.get("max_slack_Linf", 0)
        if slack > max_slack:
            max_slack = slack
            hardest_constraint = name
        if slack > 0.1:
            num_critical += 1
        elif slack > 0.01:
            num_high += 1
    ...
    return {
        "has_critical_constraints": num_critical > 0,
        "hardest_constraint": hardest_constraint,
        "max_slack": max_slack,
        "num_critical": num_critical,
        "num_high": num_high,
        "num_violations": num_critical + num_high,
    }


def format_hardness_suggestions(
    hardness_report: dict[str, dict] | None, dt: float = 0.02
) -> list[str]:
    """
    Generate specific suggestions based on constraint hardness with timestep info.
    ...
    for name, metrics in hardness_report.items():
        ...
        if active_timesteps:
            time_ranges = _identify_time_ranges(active_timesteps, dt)
            if time_ranges:
                time_info = f" (violations at {_format_time_ranges(time_ranges)})"

        # Get worst timestep info
        worst_info = ""
        worst = _find_worst_timesteps(slack_by_timestep, top_n=1, dt=dt)
        if worst:
            worst_info = f" Worst violation at t={worst[0][0]:.2f}s."

        if max_slack > 0.1:
            # Critical - generate specific fix suggestion
            if "contact_aware" in name.lower() or "llm" in name.lower():
                suggestions.append(
                    f"**{name}**: Your constraint bounds are too tight!{time_info} "
                    f"Slack = {max_slack:.3f}.{worst_info} "
                    f"Widen your bounds by at least {max_slack * 2:.2f} for those timesteps, "
                    f"or use phase-aware bounds (different for stance vs flight)."
                )
            elif "friction" in name.lower():
                suggestions.append(
                    f"**{name}**: Friction cone violated by {max_slack:.3f}.{time_info}{worst_info} "
                    f"Your motion requires excessive tangential forces. "
                    f"Adjust your contact_sequence or reduce horizontal velocity constraints."
                )
            elif "clearance" in name.lower():
                suggestions.append(
                    f"**{name}**: Body clearance violated by {max_slack:.3f}m.{time_info}{worst_info} "
                    f"Robot body is penetrating ground during rotation. "
                    f"Adjust contact_sequence timing or require higher jump height."
                )
            elif "height" in name.lower():
                suggestions.append(
                    f"**{name}**: Foot height violated by {max_slack:.3f}m.{time_info}{worst_info} "
                    f"Feet are below ground level. "
                    f"Check contact_sequence matches motion phases."
                )
            elif "complementarity" in name.lower():
                suggestions.append(
                    f"**{name}**: Complementarity violated by {max_slack:.3f}.{time_info}{worst_info} "
                    f"Contact forces exist when feet should be in flight. "
                    f"Your contact_sequence may not match the actual motion."
                )
            elif "velocity" in name.lower():
                suggestions.append(
                    f"**{name}**: Velocity constraint violated by {max_slack:.3f}.{time_info}{worst_info} "
                    f"The robot can't achieve the required velocity. "
                    f"Relax velocity bounds or extend motion duration."
                )
            elif "joint" in name.lower():
                suggestions.append(
                    f"**{name}**: Joint limit violated by {max_slack:.3f}rad.{time_info}{worst_info} "
                    f"Motion requires poses outside joint limits. "
                    f"Adjust constraints to stay within mechanical limits."
                )
            ...
            if "contact_aware" in name.lower() or "llm" in name.lower():
                suggestions.append(
                    f"**{name}**: Minor violation ({max_slack:.4f}).{time_info} "
                    f"Consider widening bounds by ~50% for those timesteps."
                )
