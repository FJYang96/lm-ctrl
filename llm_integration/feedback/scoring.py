...
TERMINAL STATE CHECK: The trajectory must end in a stable deployable posture, not necessarily at the initial x/y position or initial yaw. Inspect final height, roll/pitch, linear velocity, angular velocity, joint feasibility, and contact/landing quality. Penalize unrecoverable endings: cap at <=0.6 if terminal linear/angular velocity is not small, roll/pitch is not upright, height is not near standing height, joints are far from a feasible stance, or feet are not planted for a ground-ending motion. Do NOT penalize terminal COM x/y displacement when the command asks for translation; the robot should stay where it traveled. Do NOT penalize terminal yaw difference when the command asks for a heading change; it should hold the requested yaw instead of unwinding. For aerial rotations, require upright settled landing posture, but do not require the COM x/y or yaw to return to the exact start unless the command explicitly asks for that.
