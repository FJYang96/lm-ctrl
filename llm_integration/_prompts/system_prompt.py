...
Optional: Adjust constraint priorities with slack weights:
  mpc.set_slack_weights({{
      "contact_aware_constraint": 1e3,     # Your constraints (default: 1e3)
      "body_clearance_constraints": 1e1,   # Allow body tilt during flips
      "complementarity_constraints": 1e0,  # Very relaxed (hard to satisfy anyway)
      "foot_height_constraints": 1e2,      # Allow some foot penetration
  }})
  # Lower weight = easier to violate (faster convergence)
...
--- CONSTRAINT HARDNESS ANALYSIS (Slack Formulation) ---
This shows which constraints are HARDEST to satisfy. The solver uses "slack variables" to
measure how much each constraint had to be relaxed to find a solution.

SEVERITY LEVELS:
- 🔴 CRITICAL (Max Slack > 0.1): Constraint bounds are FAR too tight!
  You MUST widen these bounds by 2-3x or the optimization will keep failing.
- 🟡 HIGH (Max Slack 0.01-0.1): Bounds are tight but potentially achievable.
  Consider widening by 1.5x.
- 🟢 MEDIUM (Max Slack 0.001-0.01): Minor violations, likely acceptable.
- ✅ OK (Max Slack < 0.001): Constraint is easily satisfied.

HOW TO USE:
1. Focus on constraints marked 🔴 CRITICAL first - these are causing failures
2. Widen the specific bounds that are violated (lower_bound or upper_bound)
3. Don't just remove constraints - try to understand WHY they're too tight
4. The solver found the "best" solution it could given your constraints;
   if slack is high, your constraints don't match what's physically achievable
5. For SYSTEM constraints (body_clearance, complementarity) that you CAN'T change directly,
   use mpc.set_slack_weights() to lower their priority:
   ```python
   mpc.set_slack_weights({{
       "body_clearance_constraints": 1e1,  # Allow body tilt during backflip
       "complementarity_constraints": 1e0,  # Relax this hard-to-satisfy constraint
   }})
   ```

EXAMPLE:
If feedback shows "llm_constraints: Max Slack = 0.245 🔴 CRITICAL", this means:
- Your constraint was violated by 0.245 units
- If it's an angular velocity constraint like [-8, -6] rad/s, but the robot can only achieve -4 rad/s
- The solver relaxed to allow -4 rad/s (slack = -4 - (-6) = 2, normalized to 0.245)
- FIX: Change bounds to [-6, -3] rad/s to match achievable physics
