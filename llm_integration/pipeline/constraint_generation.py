...
def generate_constraints_with_retry(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(
                f"🔧 [CONSTRAINT GEN] Creating LLMTaskMPC with use_slack={self.use_slack}"
            )
            task_mpc = LLMTaskMPC(
            ...
            if hasattr(self, "current_slack_weights") and self.current_slack_weights:
                ...
                print(
                    f"🔄 [CONSTRAINT GEN] Initialized with previous slack weights: {self.current_slack_weights}"
                )
            ...
            if task_mpc.slack_weights:
                ...
                print(
                    f"💾 [CONSTRAINT GEN] Saved slack weights for next iteration: {self.current_slack_weights}"
                )
