...
class LLMTaskMPC:
    ...
    def configure_from_llm(self, llm_config_code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if current_horizon != expected_horizon:
                print(
                    f"⚠️ [CONFIG] contact_sequence dimension mismatch detected: "
                ...
                if current_horizon < expected_horizon:
                    ...
                    last_col = self.contact_sequence[:, -1:].repeat(
                        padding_needed, axis=1
                    )
                    self.contact_sequence = np.hstack([self.contact_sequence, last_col])
                    print(
                        f"✅ [CONFIG] Extended contact_sequence by {padding_needed} timesteps"
                    )
                else:
                    # Truncate
                    self.contact_sequence = self.contact_sequence[:, :expected_horizon]
                    print(
                        f"✅ [CONFIG] Truncated contact_sequence to {expected_horizon} timesteps"
                    )
        ...
        try:
            ...
            if self.use_slack:
                ...
                if self.slack_weights:
                    ...
                    print(
                        "[BUILD MPC] Using default slack weights (LLM did not customize)"
                    )
                ...
                for name, weight in self.mpc.slack_weights.items():
                    marker = (
                        "← LLM"
                        if self.slack_weights and name in self.slack_weights
                        else ""
                    )
                    print(f" {name}: {weight:.0e} {marker}")
                print(
                    f"[BUILD MPC] Slack MPC created with {len(self.mpc.slack_variables)} slack variable groups"
                )
            else:
    ...
    def solve_trajectory(
        ...
        if current_horizon != expected_horizon:
            print(
                f"[DIMENSION FIX] contact_sequence mismatch: "
            ...
            if current_horizon < expected_horizon:
                ...
                self.contact_sequence = np.hstack([self.contact_sequence, last_col])
                print(
                    f"[DIMENSION FIX] Extended contact_sequence by {padding_needed} "
                ...
                self.contact_sequence = self.contact_sequence[:, :expected_horizon]
                print(
                    f"[DIMENSION FIX] Truncated contact_sequence to {expected_horizon} timesteps"
                )
    ...
    def print_constraint_hardness_report(self) -> None:
        ...
        if self.mpc is not None and hasattr(
