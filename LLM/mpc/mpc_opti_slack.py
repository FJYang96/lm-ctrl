...
class QuadrupedMPCOptiSlack:
    ...
    DEFAULT_SLACK_WEIGHTS = {
        "friction_cone_constraints": 1e5,      # Physical - high penalty
        "foot_height_constraints": 1e4,        # Important for safety
        "foot_velocity_constraints": 1e3,      # Can be relaxed slightly
        "joint_limits_constraints": 1e5,       # Hardware limits - high penalty
        "input_limits_constraints": 1e5,       # Hardware limits - high penalty
        "body_clearance_constraints": 1e4,     # Safety constraint
        "complementarity_constraints": 1e2,    # Can be relaxed more
        "additional_constraints": 1e5,         # LLM-generated constraints
    }
    ...
    def _create_decision_variables(self) -> None:
        ...
        self.P_ref_state = self.opti.parameter(self.states_dim)
        self.P_ref_input = self.opti.parameter(self.inputs_dim)
        self.P_initial_state = self.opti.parameter(self.states_dim)

        # Robot parameters
        self.P_mu = self.opti.parameter()
        self.P_grf_min = self.opti.parameter()
        self.P_grf_max = self.opti.parameter()
        ...
        for k in range(self.horizon):
            ...
            if self.additional_constraints is not None:
                result = self.additional_constraints(
                    x_k, u_k, self.kindyn_model, self.config, contact_k, k, self.horizon
                )
                if result is not None:
                    expr, lb, ub = result
                    self._add_constraint_with_slack("additional_constraints", k, expr, lb, ub)
    ...
    def solve_trajectory(
        ...
        for i in range(self.horizon):
            ...
            for foot in range(4):
                if contact_i[foot] > 0.5:
                    U_init[12 + foot * 3 + 2, i] = self.config.robot_data.mass * 9.81 / 4 * 0.8
        ...
        try:
            sol = self.opti.solve()
            self._last_solution = sol  # Store for analysis
            ...
            try:
                ...
                joint_vel_traj = U_opt[0:12, :].T
                grf_traj = U_opt[12:24, :].T
                # Store debug accessor for slack analysis
    ...
    def analyze_constraint_hardness(self) -> dict[str, dict]:
        ...
        Returns normalized metrics to fairly compare constraints of different dimensions.
        
        Returns:
            Dictionary mapping constraint names to their hardness metrics:
            {
                "friction_cone_constraints": {
                    "total_slack_L1": float,      # Sum of all slack (raw, dimension-dependent)
                    "max_slack_Linf": float,      # Max single slack value (dimension-independent)
                    "mean_slack_per_dim": float,  # Normalized: total / num_dimensions
                    "total_dims": int,            # Total constraint dimensions across all timesteps
                    "active_timesteps": list[int],
                    "slack_by_timestep": dict[int, float],
                }
            }
        """
        if not self.use_slack:
        ...
        # Determine value accessor: use solution if available, else debug accessor
        value_fn = None
        ...
        for (constraint_name, k), (s_l, s_u, n_constraints) in self.slack_variables.items():
            # Get slack values using the appropriate accessor
            try:
        ...
        # Calculate normalized metric: mean slack per dimension
        for name in results:
            total_dims = results[name]["total_dims"]
            if total_dims > 0:
                results[name]["mean_slack_per_dim"] = results[name]["total_slack_L1"] / total_dims
            else:
                results[name]["mean_slack_per_dim"] = 0.0
        
        # Sort by max_slack_Linf (dimension-independent metric) - fairest comparison
    ...
    def print_constraint_hardness_report(self) -> None:
        ...
        if not results:
            print("No slack analysis available. Run solve_trajectory() first with use_slack=True.")
            return
        
        print("\n" + "=" * 100)
        print("CONSTRAINT HARDNESS ANALYSIS (Slack Formulation)")
        print("=" * 100)
        print(f"{'Constraint':<30} {'Max(Linf)':<12} {'Mean/Dim':<12} {'Total(L1)':<12} {'Dims':<8} {'Active':<10}")
        print("-" * 100)
        ...
        for name, m in results.items():
            ...
            # Severity indicator based on Linf (dimension-independent)
            ...
            print(f"{severity} {name:<28} {m['max_slack_Linf']:<12.6f} {m['mean_slack_per_dim']:<12.6f} "
                  f"{m['total_slack_L1']:<12.4f} {m['total_dims']:<8d} {active_str:<10}")
        
        print("=" * 100)
        
        # Summary
        print("\nLegend: 🔴 HIGH (>0.1) | 🟡 MEDIUM (>0.01) | 🟢 LOW (>1e-6) | ✅ OK")
        print("\nKey Metrics:")
        print("  - Max(Linf): Maximum single slack value - BEST for comparing hardness across constraints")
        print("  - Mean/Dim: Average slack per dimension - normalized for fair comparison")
        print("  - Total(L1): Sum of all slack - raw value, depends on constraint dimension")
        
        # Find hardest constraint by Linf
        if results:
            hardest = list(results.keys())[0]
            hardest_linf = results[hardest]["max_slack_Linf"]
            print(f"\n  Hardest constraint (by Linf): {hardest} = {hardest_linf:.6f}")

    def get_slack_trajectory(self, constraint_name: str) -> np.ndarray:
        """
        Get the slack values over the horizon for a specific constraint.
        
        Args:
            constraint_name: Name of the constraint function
            
        Returns:
            Array of shape (horizon,) with total slack at each timestep
        """
        ...
        slack_traj = np.zeros(self.horizon)
        
        for (name, k), (s_l, s_u, _) in self.slack_variables.items():
            if name == constraint_name and k < self.horizon:
                try:
                    s_l_val = np.array(value_fn(s_l)).flatten()
                    s_u_val = np.array(value_fn(s_u)).flatten()
                    slack_traj[k] = np.sum(s_l_val) + np.sum(s_u_val)
                except Exception:
                    pass
