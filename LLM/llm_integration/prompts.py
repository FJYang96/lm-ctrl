...
The solver uses a **slack formulation** to analyze how hard each constraint is to satisfy:
- Each constraint `lb <= expr <= ub` becomes `lb - s_l <= expr <= ub + s_u` with penalty `λ*(s_l² + s_u²)` in cost
- After optimization, the **slack value** s tells you how much the constraint was violated
- High slack = constraint is TOO RESTRICTIVE (solver struggling to satisfy it)
- Zero slack = constraint is easily satisfied (may even be relaxable)

**How to use hardness feedback:**
1. If you see "🔴 HIGH" hardness for a constraint → **MUST relax bounds** (widen lb/ub range)
2. If you see "🟡 MEDIUM" hardness → Consider relaxing slightly
3. If you see "✅ OK" → Constraint is fine, no change needed

**Example fix for high hardness:**
```
# BEFORE (too restrictive):
lower_bounds.append(-0.1)   # pitch in [-0.1, 0.1] - very tight!
upper_bounds.append(0.1)

# AFTER (relaxed):
lower_bounds.append(-0.5)   # pitch in [-0.5, 0.5] - more achievable
...
### ⚠️ CRITICAL FOR BACKFLIP/ROTATION MANEUVERS:

The optimizer will NOT rotate unless you FORCE it! Simply allowing pitch freedom is NOT enough.

**WHY IT FAILS**: The optimizer finds "no rotation" (stay at 0°) easier than "rotate 360° and return to 0°".

**HOW TO FIX - You MUST do ALL of these**:

1. **Maintain angular velocity THROUGHOUT flight** (not just at takeoff):
   ```python
   if is_flight:  # e.g., k >= 15 and k <= 35
       ang_vel_pitch = x_k[MP_X_BASE_ANG][1]
       constraints.append(ang_vel_pitch)
       lower_bounds.append(-12.0)  # Must keep rotating backward
       upper_bounds.append(-4.0)   # Throughout flight!
   ```

2. **Use PROGRESSIVE pitch targets** at intermediate timesteps:
   ```python
   # At 1/4 through flight: pitch should be around -π/2 (-90°)
   if k == flight_start + flight_duration // 4:
       constraints.append(pitch)
       lower_bounds.append(-2.0)  # Around -π/2
       upper_bounds.append(-1.0)
   
   # At 1/2 through flight: pitch should be around -π (-180°)
   if k == flight_start + flight_duration // 2:
       constraints.append(pitch)
       lower_bounds.append(-3.5)  # Around -π
       upper_bounds.append(-2.5)
   ```

3. **Terminal pitch should account for FULL rotation**:
   - For backflip: final pitch ≈ -2π (-6.28 rad) OR back to ~0 after completing rotation
   - The key is the ANGULAR VELOCITY constraints force the rotation to happen
"""
