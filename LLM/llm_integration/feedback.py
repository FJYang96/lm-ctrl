...
class TrajectoryAnalyzer:
    ...
    def analyze_trajectory(
        ...
        # Note: rotation_target_achieved disabled - not all maneuvers require rotation
        metrics.overall_success = (
            metrics.optimization_converged
            and metrics.height_clearance_achieved
            and metrics.rotation_target_achieved  # Disabled
...
class FeedbackGenerator:
    ...
    def generate_feedback(
        ...
        # Identify key issues
        issues = self._identify_issues(metrics, hardness_report)
        suggestions = self._generate_suggestions(metrics, issues, hardness_report)
...
{self._format_hardness_report(hardness_report)}
...
"""
    ...
    def _format_hardness_report(self, hardness_report: Dict[str, Dict]) -> str:
        """Format constraint hardness analysis for feedback."""
        if not hardness_report:
            return ""
        
        lines = ["\n## Constraint Hardness Analysis (from Slack Formulation)\n"]
        lines.append("This shows which constraints are HARDEST to satisfy.")
        lines.append("- **Max Slack (Linf)**: Maximum single violation - best for comparing different constraints")
        lines.append("- **Mean/Dim**: Average slack per dimension - normalized metric\n")
        lines.append("| Constraint | Max Slack | Mean/Dim | Severity | Recommendation |")
        lines.append("|------------|-----------|----------|----------|----------------|")
        
        for name, metrics in hardness_report.items():
            # Use Linf (max slack) for severity - dimension-independent metric
            max_slack = metrics.get("max_slack_Linf", metrics.get("max_violation", 0))
            mean_per_dim = metrics.get("mean_slack_per_dim", metrics.get("mean_violation", 0))
            
            # Severity based on Linf (dimension-independent, fair comparison)
            if max_slack > 0.1:
                severity = "🔴 HIGH"
                recommendation = "MUST relax bounds significantly"
            elif max_slack > 0.01:
                severity = "🟡 MEDIUM"
                recommendation = "Consider relaxing bounds"
            elif max_slack > 0.001:
                severity = "🟢 LOW"
                recommendation = "Minor adjustment may help"
            else:
                severity = "✅ OK"
                recommendation = "No change needed"
            
            # Shorten constraint name for readability
            short_name = name.replace("_constraints", "")
            lines.append(f"| {short_name:<25} | {max_slack:>9.5f} | {mean_per_dim:>8.5f} | {severity:<8} | {recommendation} |")
        
        lines.append("\n**Key Insight**: Focus on constraints marked 🔴 HIGH - the Max Slack value indicates the worst single violation.\n")
    ...
    def _identify_issues(
        ...
        """Identify primary issues from metrics and hardness analysis"""
        issues = []
        ...
        # Add issues from hardness analysis (use Linf for fair comparison across dimensions)
        if hardness_report:
            for name, data in hardness_report.items():
                # Use max_slack_Linf for severity - dimension-independent metric
                max_slack = data.get("max_slack_Linf", data.get("max_violation", 0))
                mean_per_dim = data.get("mean_slack_per_dim", 0)
                if max_slack > 0.1:
                    issues.append(
                        {
                            "category": "constraint_hardness",
                            "severity": "high",
                            "description": f"Constraint '{name}' is too restrictive",
                            "details": f"Max slack: {max_slack:.4f}, Mean/dim: {mean_per_dim:.4f}. MUST relax bounds.",
                        }
                    )
                elif max_slack > 0.01:
                    issues.append(
                        {
                            "category": "constraint_hardness",
                            "severity": "medium",
                            "description": f"Constraint '{name}' is somewhat restrictive",
                            "details": f"Max slack: {max_slack:.4f}. Consider relaxing bounds.",
                        }
                    )
    ...
    def _generate_suggestions(
        ...
        for issue in issues:
            if issue["category"] == "optimization":
                ...
                suggestions.append("Widen lb/ub ranges for hard constraints")
            ...
            elif issue["category"] == "constraint_hardness":
                suggestions.append(
                    f"⚠️ {issue['description']} - widen the bounds significantly"
                )
