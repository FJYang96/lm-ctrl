...
def check_api_key() -> tuple[bool, str]:
    ...
    try:
        ...
        if not api_key or api_key == "your_api_key_here":
            return False, "ANTHROPIC_API_KEY not configured. Please set it in the .env file."
...
class FrontendLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            iter_match2 = re.search(r'Iteration\s+(\d+)\s+completed', msg, re.IGNORECASE)
            if iter_match2:
...
def run_pipeline_thread(command: str, max_iterations: int, config_mode: str, results_dir: str, solver_max_iter: int | None = None):
...
def run_pipeline():
    ...
    solver_max_iter = int(solver_max_iter_raw) if solver_max_iter_raw and str(solver_max_iter_raw).strip() else None
...
def get_status():
    ...
    return jsonify({
        ...
        "elapsed_time": round(time.time() - pipeline_status["start_time"], 1) if pipeline_status["start_time"] and pipeline_status["running"] else None
    })
...
def list_videos():
        ...
        if video_path.exists():
            for video_file in sorted(video_path.glob("*.mp4")):
                ...
                if "simulation_iter" in name:
                    ...
                    iteration = name.replace("simulation_iter_", "").replace("_DEBUG", "").replace(".mp4", "")
                elif "planned_traj_iter" in name:
                    video_type = "planned"
                    iteration = name.replace("planned_traj_iter_", "").replace("_DEBUG", "").replace(".mp4", "")
                    if "_DEBUG" in name:
                        video_type = "planned-debug"
                elif "debug_trajectory_iter" in name:
                    video_type = "debug"
                    iteration = name.replace("debug_trajectory_iter_", "").replace("_DEBUG", "").replace(".mp4", "")
                else:
                    video_type = "other"
                    iteration = "0"
    ...
    type_order = {"simulation": 0, "planned": 1, "planned-debug": 2, "debug": 3, "other": 4}
    videos.sort(key=lambda v: (-int(v["iteration"]) if v["iteration"].isdigit() else 0, type_order.get(v["type"], 4)))
...
def serve_video_by_name(filename):
...
def get_latest_iteration_video():
            ...
            for video_file in video_path.glob(pattern):
                ...
                try:
                    ...
                    if video_type == "planned":
                        iter_part = name.replace("planned_traj_iter_", "").replace("_DEBUG", "").replace(".mp4", "")
                    elif video_type == "debug":
                        iter_part = name.replace("debug_trajectory_iter_", "").replace(".mp4", "")
                    else:  # simulation
                        iter_part = name.replace("simulation_iter_", "").replace(".mp4", "")
                    
                    iteration = int(iter_part)
                    
                    # Prefer planned > simulation > debug for same iteration
                    type_priority = {"planned": 3, "simulation": 2, "debug": 1}
                    current_priority = type_priority.get(video_type, 0)
                    existing_priority = 0
                    if latest_video:
                        existing_priority = type_priority.get(latest_video.get("type", ""), 0)
                    
                    # Update if this is a newer iteration, or same iteration with higher priority
                    if iteration > latest_iteration or (iteration == latest_iteration and current_priority > existing_priority):
...
def serve_video_from_results(filename):
