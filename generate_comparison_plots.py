...
def compute_both_implementations(kinodynamic_model, state_traj, grf_traj, joint_vel_traj):
...
def create_comparison_plot(torques_original, torques_improved, save_path="results/inverse_dynamics_comparison.png"):
    """Create comparison plot in the style of existing debug plots"""
    
    # Create figure with 3x4 subplots (12 joints)
    fig, axes = plt.subplots(3, 4, figsize=(16, 12))
    fig.suptitle("Inverse Dynamics Comparison: Original vs Improved Implementation", fontsize=16, fontweight='bold')
    ...
    for i in range(12):
        ...
        ax.plot(time_steps, torques_original[:, i], 'b-', linewidth=2, label='original', alpha=0.8)
        ax.plot(time_steps, torques_improved[:, i], 'r-', linewidth=2, label='improved', alpha=0.8)
...
def create_difference_plot(torques_original, torques_improved, save_path="results/inverse_dynamics_differences.png"):
    ...
    # Create figure with 3x4 subplots (12 joints)
    fig, axes = plt.subplots(3, 4, figsize=(16, 12))
    fig.suptitle("Inverse Dynamics Differences: Improved - Original", fontsize=16, fontweight='bold')
    ...
    for i in range(12):
        ...
        ax.text(0.02, 0.98, f'RMSE: {rmse_joint:.3f}', transform=ax.transAxes, 
...
def create_statistics_plot(torques_original, torques_improved, time_original, time_improved, 
    ...
    fig.suptitle("Inverse Dynamics Statistical Analysis", fontsize=16, fontweight='bold')
    ...
    width = 0.35
    ax3.bar(joints - width/2, max_orig, width, label='Original', alpha=0.7, color='blue')
    ax3.bar(joints + width/2, max_improved, width, label='Improved', alpha=0.7, color='red')
    ...
    for i, time_val in enumerate(times):
        ax4.text(i, time_val + 0.01, f'{time_val:.3f}s', ha='center', va='bottom', fontweight='bold')
...
def print_summary_statistics(torques_original, torques_improved, time_original, time_improved):
...
def main():
    parser = argparse.ArgumentParser(description='Generate comparison plots for inverse dynamics implementations')
    ...
    torques_original, torques_improved, time_original, time_improved = compute_both_implementations(
    ...
    comparison_path = create_comparison_plot(
        torques_original, torques_improved, 
    ...
    # Print summary
    print_summary_statistics(torques_original, torques_improved, time_original, time_improved)
    
    # Save data for future use
    np.save(os.path.join(args.output_dir, "torques_original_comparison.npy"), torques_original)
    np.save(os.path.join(args.output_dir, "torques_improved_comparison.npy"), torques_improved)
