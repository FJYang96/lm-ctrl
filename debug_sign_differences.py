...
def analyze_sign_patterns():
    ...
    for joint in range(12):
        ...
        print(f"Joint {joint+1:2d}: Same sign: {same_sign:2d}/{total} ({same_sign/total*100:.1f}%), "
    ...
    print(f"\n🎯 Overall Analysis:")
    print(f"Same sign: {same_sign_total}/{total_elements} ({same_sign_total/total_elements*100:.1f}%)")
    print(f"Opposite sign: {opposite_sign_total}/{total_elements} ({opposite_sign_total/total_elements*100:.1f}%)")
    
    overall_correlation = np.corrcoef(torques_orig.flatten(), torques_improved.flatten())[0, 1]
    print(f"Overall correlation: {overall_correlation:.3f}")
    
    # Check if it's a simple sign flip
    sign_flipped_correlation = np.corrcoef(torques_orig.flatten(), -torques_improved.flatten())[0, 1]
...
def create_sign_analysis_plot(torques_orig, torques_improved):
    ...
    joint_agreements = []
    for joint in range(12):
        same_sign = np.sum(np.sign(torques_orig[:, joint]) == np.sign(torques_improved[:, joint]))
    ...
    colors = ['red' if c < -0.5 else 'orange' if c < 0 else 'lightgreen' if c < 0.5 else 'green' for c in correlations]
    ...
    ax4.plot(time_steps, torques_orig[:, worst_joint], 'b-', label=f'Original Joint {worst_joint+1}')
    ax4.plot(time_steps, torques_improved[:, worst_joint], 'r-', label=f'Improved Joint {worst_joint+1}')
    ax4.plot(time_steps, -torques_improved[:, worst_joint], 'g--', label=f'-(Improved) Joint {worst_joint+1}')
    ax4.set_xlabel('Time Step')
    ax4.set_ylabel('Torque (Nm)')
    ax4.set_title(f'Example: Joint {worst_joint+1} (Correlation: {correlations[worst_joint]:.3f})')
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig("results/sign_analysis.png", dpi=150, bbox_inches='tight')
    print(f"Sign analysis plot saved to: results/sign_analysis.png")
