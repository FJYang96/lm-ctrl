...
# Import the KinoDynamic_Model to access the underlying KinDynComputations object
...
def compute_joint_torques(
    ...
    pos_kindyn_traj = np.hstack([pos_kindyn_traj, state_traj[:, 12:24]]) # Add joint positions
    ...
    wrench_ext = J_FL_sym.T @ F_FL_sym + J_FR_sym.T @ F_FR_sym + \
                 J_RL_sym.T @ F_RL_sym + J_RR_sym.T @ F_RR_sym
    ...
    inverse_dynamics_fun = cs.Function(
        'inverse_dynamics',
        [base_pos_sym, base_rpy_sym, joint_pos_sym, base_vel_sym, joint_vel_sym, base_acc_sym, joint_acc_sym, f_ext_sym],
        [tau_full_sym],
    )
    ...
    for i in range(num_steps):
        ...
        tau_full = inverse_dynamics_fun(
            base_pos_vec, base_rpy_vec, joint_pos_vec, 
            base_vel_vec, joint_vel_vec, 
            base_acc_vec, joint_acc_vec, 
            grfs_vec
        )
