...
class HoppingMPCOpti:
        ...
        for k in range(self.horizon):
            ...
            for joint_idx in range(12):
                self.opti.subject_to(
                    joint_positions[joint_idx] >= joint_limits_lower[joint_idx]
                )
                self.opti.subject_to(
                    joint_positions[joint_idx] <= joint_limits_upper[joint_idx]
                )

            # Input bounds (already there but let's make them more reasonable)
            self.opti.subject_to(
                self.opti.bounded(-10, u_k[0:12], 10)
    ...
    def _add_friction_cone_constraints(self, u_k, contact_k, k):
        ...
        for foot_idx in range(4):
            ...
            self.opti.subject_to(
                f_x >= -large_bound * (1 - contact_flag) - f_max * contact_flag
            )
            self.opti.subject_to(
                f_x <= large_bound * (1 - contact_flag) + f_max * contact_flag
            )
            self.opti.subject_to(
                f_y >= -large_bound * (1 - contact_flag) - f_max * contact_flag
            )
            self.opti.subject_to(
                f_y <= large_bound * (1 - contact_flag) + f_max * contact_flag
            )
            self.opti.subject_to(
                f_z <= large_bound * (1 - contact_flag) + f_max * contact_flag
            )
    ...
    def _add_foot_height_constraints(self, x_k, contact_k, k):
        ...
        foot_height_fl = self.kindyn_model.forward_kinematics_FL_fun(
            H, joint_positions
        )[2, 3]
        foot_height_fr = self.kindyn_model.forward_kinematics_FR_fun(
            H, joint_positions
        )[2, 3]
        foot_height_rl = self.kindyn_model.forward_kinematics_RL_fun(
            H, joint_positions
        )[2, 3]
        foot_height_rr = self.kindyn_model.forward_kinematics_RR_fun(
            H, joint_positions
        )[2, 3]
        ...
        for foot_idx in range(4):
            ...
            upper_bound = stance_upper_bound * contact_flag + swing_height_max * (
                1 - contact_flag
            )

            self.opti.subject_to(
                self.opti.bounded(lower_bound, foot_height, upper_bound)
            )
    ...
    def _add_foot_velocity_constraints(self, x_k, u_k, contact_k, k):
        ...
        foot_vel_FL = (
            self.kindyn_model.jacobian_FL_fun(H, joint_positions)[0:3, :] @ qvel
        )
        foot_vel_FR = (
            self.kindyn_model.jacobian_FR_fun(H, joint_positions)[0:3, :] @ qvel
        )
        foot_vel_RL = (
            self.kindyn_model.jacobian_RL_fun(H, joint_positions)[0:3, :] @ qvel
        )
        foot_vel_RR = (
            self.kindyn_model.jacobian_RR_fun(H, joint_positions)[0:3, :] @ qvel
        )
        ...
        for foot_idx in range(4):
            ...
            for axis in range(3):
                ...
                lower_bound = -stance_tolerance * contact_flag - large_bound * (
                    1 - contact_flag
                )
                upper_bound = stance_tolerance * contact_flag + large_bound * (
                    1 - contact_flag
                )
